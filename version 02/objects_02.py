...
class AABB:
    ...
    def update(self, g, dt):
        if not self.is_static:
            if self.dx>0: self.dx-=self.air_friction
            elif self.dx==0 or abs(self.dx)<self.air_friction*1.5: pass
            else: self.dx+=self.air_friction

            if self.dy>0: self.dy-=self.air_friction
            elif self.dy==0 or abs(self.dy)<self.air_friction*1.5: pass
            else: self.dy+=self.air_friction
